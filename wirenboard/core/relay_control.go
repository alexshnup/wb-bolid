...
func status_serial(result []byte, ok uint8, relay uint8) (uint8, string) {
	fmt.Printf("%x\n", result)
	switch ok {
	case 0:
...
func Relay_ON(addr uint8, relay uint8) string {
	...
	_, out = status_serial(write_serial(crc8dallas(CommandEnter)), 0, relay)
	return out
}
